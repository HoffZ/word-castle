...
test('hit uses earlier collections exclusively when available', () => {
  assert.deepEqual(bossVocabulary([older, current], current), older.words);
});
test('hit falls back to current homework without older words', () => {
...
test('hit word selection avoids consecutive repeats when possible and supports one word', () => {
  assert.equal(pickBossWord([...older.words, ...current.words], 'dog', () => 0).id, 'cat');
  assert.equal(pickBossWord(current.words, 'cat').id, 'cat');
});
test('hit survives two hits and dies on exactly the third', () => {
