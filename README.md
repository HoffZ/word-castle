...
- Zombies spawn immediately and then initially every 10 seconds. Two consecutive hits before 35% of the walk reduce the spawn interval by one second, down to a minimum of five seconds. A hit at 65% or later increases it by one second, up to ten. Each round starts at ten seconds; boss timing is unchanged. Each takes 20 seconds to reach the castle. Multiple zombies can approach together; reaching the castle ends the round.
...
- Every word must be answered correctly three times. Completing the weekly words clears remaining zombies and triggers four seconds of confetti before a single large boss arrives. The boss takes three hits and uses words from older collections, falling back to the current collection when needed. It has 20 seconds of walking time; movement pauses during cannonball flights. The first two hits trigger random ouch sounds and an “Uff og huff!” speech bubble; the final hit triggers a comic howl and victory. Retry resets practice progress only. Total points persist in LocalStorage under `word-castle.total-score` across rounds, losses, cancellations and reloads.
- Cannon shots use locally generated Web Audio effects. Hits randomly mix bundled recordings from `src/sounds/ouch/` with a silly Norwegian reaction when an on-device Norwegian speech voice is available, otherwise a synthesized cartoon yelp. Supported recordings (`.mp3`, `.wav`, `.ogg`, `.m4a`) in that folder are included automatically by Vite and preloaded when a round starts. The boss shares the random hit sounds, keeps its “Uff og huff!” speech bubble, and has a distinct death howl. Victory adds a short fanfare and confetti pops; game over plays a short descending cartoon wah-wah. The sound toggle persists across rounds; pausing or leaving the game stops audio.
